...
const Ammo = () => {
  ...
  const { data, isLoading } = useFetch(`ammos/${id}`);

  return (
    <Box mx={{ md: 10, xs: 2 }} my={2}>
      <CustomBreadcrumbs category="ammos" name={data?.name} />
            ...
            <Grid item lg={4} md={7} sm={12} xs={12}>
              ...
              >
                ...
                <Box display="flex" flexDirection="column" width="100%" mb={2}>
                  ...
                  <Divider sx={{ my: 2 }} color="#eee" variant="middle" />
                  {data?.attackPower?.length > 0 && (
                    <Box px={2}>
                      <Typography align="center" gutterBottom>
                        Attack Power
                      </Typography>
                      <CustomTable data={data.attackPower} name="Damage Type" />
                    </Box>
                  )}
                </Box>
              </Paper>
