...
const Weapon = () => {
  const { id } = useParams();
  const { data, isLoading } = useFetch(`weapons/${id}`);

  return (
    <Box mx={{ md: 10, xs: 2 }} my={2}>
      <CustomBreadcrumbs category="weapons" name={data?.name} />
      {!isLoading && (
        <Paper sx={{ pb: 2 }}>
                ...
                <Box display="flex" flexDirection="column" width="100%" mb={2}>
                  ...
                  {data?.attack?.length > 0 && (
                    <Box px={2}>
                      ...
                      <CustomTable data={data.attack} name="Damage Type" />
                    </Box>
                  )}
                  <Divider sx={{ my: 2 }} color="#eee" variant="middle" />
                  {data?.defence?.length > 0 && (
                    <Box px={2}>
                      ...
                      <CustomTable data={data.defence} name="Defence Type" />
                    </Box>
                  )}
                  <Divider sx={{ my: 2 }} color="#eee" variant="middle" />
                  {data?.requiredAttributes?.length > 0 && (
                    <Box px={2}>
                      <Typography align="center" gutterBottom>
                        Requirements
                      </Typography>
                      <CustomTable data={data.requiredAttributes} name="Stat" />
                    </Box>
                  )}
                  <Divider sx={{ my: 2 }} color="#eee" variant="middle" />
                  {data?.scalesWith?.length > 0 && (
                    <Box px={2}>
                      <Typography align="center" gutterBottom>
                        Scaling
                      </Typography>
                      <CustomTable data={data.scalesWith} name="Stat" />
                    </Box>
                  )}
                </Box>
              </Paper>
